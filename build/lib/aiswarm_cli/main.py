...
def init(test_command, port, default_branch):
    ...
    if not os.path.exists(config_file):
        ...
        config_data = {
            ...
            "mcp_jetbrains_server": f"http://localhost:{port}",
...
def task(prompt, from_branch, test_file, test_action):
    ...
    try:
        ...
        # Read gemini.md content
        gemini_md_path = os.path.join(os.getcwd(), "gemini.md")
        gemini_md_content = ""
...
"""
        ...
        final_context_content = gemini_md_content + "\n---\n\n" + context_content

        with open(context_file, 'w', encoding='utf-8') as f:
            f.write(final_context_content)
...
@click.option('--action', default='apply_refactoring', help='The refactoring action to perform (e.g., apply_refactoring, organize_imports).')
