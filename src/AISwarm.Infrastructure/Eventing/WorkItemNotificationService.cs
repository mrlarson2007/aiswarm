...
public class WorkItemNotificationService(IEventBus<TaskEventType, ITaskLifecyclePayload> bus)
    : BaseNotificationService<TaskEventType, ITaskLifecyclePayload, TaskEventEnvelope>(bus),
        IWorkItemNotificationService
{
    public IAsyncEnumerable<TaskEventEnvelope> SubscribeForAgent(string agentId, CancellationToken ct = default)
    {
        ...
        var filter = new TaskEventFilter(
            ...
            e => e.Payload is TaskCreatedPayload p && p.AgentId == agentId);
    ...
    public IAsyncEnumerable<TaskEventEnvelope> SubscribeForPersona(string persona, CancellationToken ct = default)
    {
        ...
        var filter = new TaskEventFilter(
            [TaskEventType.Created],
            e => e.Payload is TaskCreatedPayload p
    ...
    public IAsyncEnumerable<TaskEventEnvelope> SubscribeForAgentOrPersona(string agentId, string persona,
        CancellationToken ct = default)
    {
        EventValidation.ValidateRequiredId(agentId, nameof(agentId));
        EventValidation.ValidateRequiredId(persona, nameof(persona));
        ...
        CancellationToken ct = default)
    {
        ...
        var filter = new TaskEventFilter(
            [TaskEventType.Completed, TaskEventType.Failed],
            e => taskIds.Contains(e.Payload.TaskId));
        return ToTaskEventEnvelopeAsyncEnumerable(Bus.Subscribe(filter, ct));
    }
    ...
    {
        var filter = new TaskEventFilter(
            [TaskEventType.Created, TaskEventType.Claimed, TaskEventType.Completed, TaskEventType.Failed]);
        return ToTaskEventEnvelopeAsyncEnumerable(Bus.Subscribe(filter, ct));
    }

    public async Task<string?> TryConsumeTaskCreatedAsync(string agentId, string persona,
        CancellationToken ct = default)
    {
        ...
        await using var enumerator =
            SubscribeForAgentOrPersona(agentId, persona, cts.Token).GetAsyncEnumerator(cts.Token);
        try
        {
    ...
    protected override TaskEventEnvelope CreateEventEnvelope(TaskEventType type, DateTimeOffset timestamp,
