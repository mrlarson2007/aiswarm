...
public class ListWorktreesCommandHandlerTests
{
    ...
    {
        _process.Enqueue("git", a => a.StartsWith("rev-parse --git-dir"), new AgentLauncher.Services.External.ProcessResult(false, string.Empty, "fatal: not a git repo", 128));
    ...
    public async Task WhenWorktreesExist_ShouldListEachAndReturnTrue()
    {
        ...
        _process.Enqueue("git", a => a.StartsWith("rev-parse --git-dir"), new AgentLauncher.Services.External.ProcessResult(true, ".git", string.Empty, 0));
        _process.Enqueue("git", a => a.StartsWith("worktree list"), new AgentLauncher.Services.External.ProcessResult(true, porcelain, string.Empty, 0));
        var result = await SystemUnderTest.RunAsync();
        result.ShouldBeTrue();
    ...
    public async Task WhenExceptionThrown_ShouldLogErrorAndReturnFalse()
    {
        ...
        _process.Enqueue("git", a => a.StartsWith("worktree list"), new AgentLauncher.Services.External.ProcessResult(false, string.Empty, "unexpected failure", 1));
        var result = await SystemUnderTest.RunAsync();
